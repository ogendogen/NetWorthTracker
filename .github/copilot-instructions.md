...
- Login is database-backed through the initial PostgreSQL `users` table and BCrypt password verification. Registration persists BCrypt-hashed users and emails a confirmation link; `GET /confirm-email?token=...` confirms the address and redirects to the public SPA `/email-confirmed` page. Registration does not issue a session, perform server-side input validation, or map duplicate-user exceptions to stable HTTP errors. Do not present those gaps, the committed development signing key, or mock `/data` values as production-ready behavior.
