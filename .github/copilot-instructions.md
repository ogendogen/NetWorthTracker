...
- Login is database-backed through the initial PostgreSQL `users` table and BCrypt password verification, but registration is still a no-op scaffold. Do not present registration, the committed development signing key, or mock `/data` values as production-ready behavior.
