...
- Login is database-backed through the initial PostgreSQL `users` table and BCrypt password verification. Registration persists BCrypt-hashed users, returns a JSON success response without issuing a session, and emails a confirmation link; the SPA sends successful registration to the public `/registration-success` page, which tells the user to click the link within 24 hours. `GET /confirm-email?token=...` confirms the address and redirects to the public SPA `/email-confirmed` page. Registration does not perform server-side input validation or map duplicate-user exceptions to stable HTTP errors. Do not present those gaps, the committed development signing key, or mock `/data` values as production-ready behavior.
- The API uses Microsoft.Extensions.Logging with console output and OpenTelemetry OTLP/HTTP log export to Seq, configured by `Seq:ApiUrl` and `Seq:ApiKey`. Configure resources, scope inclusion, and formatted messages in the outer `AddOpenTelemetry` callback; keep exporter settings inside `AddOtlpExporter`. Activity scopes include `TraceId`, `SpanId`, and `ParentId` for request-log correlation. Keep `HttpRequestLoggingMiddleware` first in the pipeline so completion logs capture the HTTP method, path, final status code, and elapsed time for all downstream outcomes; do not log query strings or request/response bodies. Do not claim audit logging, database logging, or request-span export is implemented.
