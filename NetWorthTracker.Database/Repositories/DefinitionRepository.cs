...
public class DefinitionRepository : IDefinitionRepository
{
    ...
    public async Task<Result> SyncUserDefinitions(int userId, IEnumerable<Definition> definitions, CancellationToken cancellationToken = default)
    {
        definitions = definitions.Select(def => new Definition() { Name =  def.Name, User = def.User, UserId = def.UserId, Type = def.Type});
        var existing = await _context.Definitions.Where(x => x.UserId == userId).ToListAsync(cancellationToken);
